...
const statusChalk = (obj) => obj.status >= 400 ? chalk.white.bgRed.bold(obj.status) : chalk.white.bgGreen.bold(obj.status);
const statusMsgChalk = (obj) => obj.statusText === 'Fail' ? chalk.white.bgRed.bold(obj.statusText) : chalk.white.bgGreen.bold(obj.statusText);
...
function extractLinksPrint(array) {
  ...
  array.forEach((obj) => {
    printLinks = `${pathChalk(obj.path)} ${hrefhChalk(obj.href)} ${textChalk(obj.text)}\n` + printLinks;
  });
  return printLinks;
}
...
const validatestLinksPrint = (array) => {
  ...
  // const array = extractLinks(route);
  array.forEach((obj) =>{
    printLinks = `${pathChalk(obj.path)} ${hrefhChalk(obj.href)} ${statusChalk(obj)} ${statusMsgChalk(obj)} ${textChalk(obj.text)} \n` + printLinks;
  });
  return printLinks;
};

module.exports = {greeting, extractLinksPrint, validatestLinksPrint, msg, whiteLetter, blackLetter, pathChalk, hrefhChalk, textChalk, error, warning};
