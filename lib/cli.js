...
const {greeting, extractLinksPrint, validatestLinksPrint, msg,  whiteLetter, blackLetter, pathChalk, hrefhChalk, textChalk, error, warning} = require("./style.js");
const process = require('process');
// const {extractLinks, validateLinks} = require('./util.js');
const { infoStats, inftoValidateStats, totalChalkBg, uniqueChalkBg, brokenChalkBg} = require('./stats.js');
...
${whiteLetter('You can to solve if a path of .md files contains links. Write md-links --help for help')}
${blackLetterLine}
...
${pathChalk('file')} ${hrefhChalk('href')} ${textChalk('text')} ${textChalk('status')} ${textChalk('statusText')}

${whiteLetter('3. Write md-links path --stats: You get stats links')}
${totalChalkBg('Total')} 
${uniqueChalkBg('Unique')} 

${whiteLetter('4. Write md-links path --stats --validate: You get stats validated links')}
${totalChalkBg('Total')} 
${uniqueChalkBg('Unique')}
...
const cli = (route, validate, stats ) => {
    ...
    if (validate && stats) {
        ...
        .catch(() => {console.log (error('There was a mistake'))});
    }
    if (stats) {
        return mdLinks (route, { validate } )
    ...
    if ( validate ) {
        ...
        .catch(() => {console.error(error('There was a mistake'))});
    }    
    return mdLinks (route, { validate })
    .then((links) => 
    {
